...
- `色モード` は `単色` と `グラデーション` を切り替えられます
- `グラデーション` では `各 stroke`、`object`、`画面全体` を基準にして、角度、幅倍率、位相、2〜4 個の color stop を調整できます
- `画面全体` は preview rect ではなく元動画 frame 基準なので、preview と export で流れ方が揃います
- グラデーションを編集しても、`単色` に戻したときの元色は保持されます
- アウトライン、ドロップシャドウ、グロー、ブレンドモードも右ペインから調整できます
- 色 picker は色相変更用で、透明度は `不透明度` スライダー 1 つに統一されています
- グラデーション有効中でも、アウトライン / ドロップシャドウ / グローは単色のままです。`先端アクセント` の `インク色` は gradient の中点色を代表色として使います
...
- style / entrance の各項目と `後段演出` chain には小さい `↺` ボタンがあります。preset を継承中の項目では disabled、上書きした項目だけ enabled になり、その項目だけ preset 値へ戻せます
- フォント、テンプレート、ガイド傾き、基本スタイル、outline / drop shadow / glow、entrance 設定、`後段演出` chain は `settings.json5` に保存され、次回起動時に復元されます
...
- `オブジェクト一覧` は `page -> group -> object -> stroke` の順でまとまっています
- `auto-follow` を ON にすると、現在 page や選択中 row が一覧内で見失いにくくなります
- `現在 page のみ` を ON にすると、今の page だけへ絞って確認できます
...
- `生存中` は current time の page にいて、まだ clear されていない object の目印です

### 3.5 ページイベント

- `ページイベント` タブでは、page ごとの開始位置と、その page を閉じる clear event を確認できます
- `現在 page のみ` を ON にすると、今の page だけへ絞って見られます
- `この時刻へ移動` を押すと、その clear event の位置へシークします
...
- project ごとに、現在の基本スタイル snapshot、現在の出現設定 snapshot、`後段演出` chain、選択 preset ID、style/entrance の継承状態、テンプレート文字列 / font / font size / 字間 / 行間 / かな倍率 / 英字倍率 / 句読点倍率 / 傾き / underlay、ガイド傾き / 次文字字間が保存されます
...
- `基本スタイル` の `色モード` を `グラデーション` にすると、線形 gradient editor が開きます
- `各 stroke` は 1 画ごと、`object` はその object 全体、`画面全体` は動画 frame 全体に対して gradient を張ります
- `繰り返し` は `なし` / `反復` / `ミラー` を選べます
- `幅倍率` は 1 周期の長さ、`位相` は周期単位の送り量です
...
- `後段演出` では、`出現中`、`画後`、`文字後`、`グループ後`、`run 後` のどこで effect を起こすかを row ごとに選べます
- row の種類は `何もしない`、`即時スタイル変更`、`時間つきスタイル変更`、`パルス発光`、`点滅` です。`追加 / 削除 / ↑ / ↓` で順序も変えられます
- `即時スタイル変更` と `時間つきスタイル変更` は target style を持ち、色、太さ、不透明度、outline / glow / shadow、ブレンドモードを個別に指定できます
- `パルス発光` と `点滅` は回数と時間を持ちます。preset 保存、project 保存、再起動復元にも含まれます
...
- `後段演出` の style target editor は v1.0 では単色・太さ・不透明度・outline / glow / shadow・ブレンドモード中心です。gradient target の個別 editor はまだありません
