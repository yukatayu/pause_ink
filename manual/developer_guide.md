...
`RevealHeadColorSource::PresetAccent` は v1.0 では独立 accent palette を持たず、Glow / Outline / DropShadow の代表色を優先し、無ければ stroke color へ fall back します。post-action chain は `evaluate_post_actions()` の中で `DuringReveal / AfterStroke / AfterGlyphObject / AfterGroup / AfterRun` を評価し、GUI 側では `後段演出` の row editor から `active_post_actions` を selection/current object へ history 経由で適用します。現在の残項目は clear / combo preset の専用 UI と portable sidecar packaging です。
...
- 横方向は [main.rs](/home/yukatayu/dev/pause_ink/crates/app/src/main.rs) の `layout_template_line()` が `egui` shaping を使って `natural_start_x` と `width` を求めます。ここは kerning を壊さないため維持します
- 縦方向は `pauseink-fonts` の `load_font_vertical_metrics()` で `ascent / descent / x-height / cap-height` を抽出し、`pauseink-template-layout` の `template_slot_vertical_metrics()` で line box 内 offset に変換します
- metrics が取れない family や system default family は fallback で「縮小文字を下揃え」にします
- `pauseink-app` は `template_font_metrics` cache を持ち、font 切替や font list rebuild、project/settings restore 時に再計算します
- guide/grid や `.pauseink` save format には metrics を保存しません。slot geometry の再計算だけで吸収します
...
- template text editor は UI 上は multiline で、改行をそのまま `template.text` へ保存する。editor height は `.pauseink` ではなく `settings.json5` の app-only `editor_ui_state` にだけ保存し、workspace ごとの快適性と project portability を分けている
- wide control は `inline_wide_control_width()` を使い、seek bar や preset ID/名の text field だけを `available_width - reserved_inline_width` で伸ばす。ボタン列や短い数値欄は fixed のままにしている
- template の `current_slot_index` は内部 state としては残るが、v1.0 の UI では露出しない。`前スロット/次スロット`、current slot 強調、`slot x/y` 表示は削除し、guide/grid の現在仕様とは切り離して扱う
