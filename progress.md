...
- 現在の即時マイルストーン: コード全体の境界監査を実施し、保存互換と command rollback の回帰を補強した。次の主対象は `V1-04 clear effect / clear preset / combo preset`、`PKG-01 portable FFmpeg sidecar packaging`、`PKG-02 GitHub Release sidecar 統合`。
- 最新の確認事項:
  - コード全体を監査し、大きい設計変更は入れず、実害が出やすい `CommandBatch` rollback、app 経由の unknown field preservation、preview 用 `RenderRequest` 組み立て境界の 3 点に絞って補強した
  - `crates/domain/src/lib.rs` に `batch_post_action_commands_update_multiple_objects_and_undo` と `command_batch_rolls_back_prior_commands_when_later_command_fails` を追加し、`post_actions` の batch 更新履歴と `CommandBatch` の途中失敗 rollback を固定した
  - `crates/project_io/src/lib.rs` では extra field 収集を helper 化しつつ、`sync_annotation_project_preserves_known_entity_extra_fields` を group / clear event まで広げて entity extra preservation を補強した
  - `crates/app/src/lib.rs` に `save_reopen_preserves_unknown_fields_across_project_sections` を追加し、`AppSession -> save_project_to_path -> load_project_from_path` 経由でも top-level / project / media / settings / pages / presets / group / clear event の未知フィールドが残ることを固定した
  - `crates/app/src/main.rs` では preview overlay 用 `RenderRequest` を `build_preview_render_request()` へ寄せ、`preview_render_request_helper_carries_time_batch_and_clamped_sizes` で paused preview batch と size clamp を回帰固定した
  - 今回の確認として `cargo fmt --all`、`cargo test --workspace`、`cargo check -p pauseink-app --all-targets`、`git diff --check` を通した
  ...
  - preset reset UI は別行の `presetへ戻す` ボタンと `継承中 / 上書き中` 表示を廃止し、各 slider / picker / combo の横に小さい `↺` アイコンを置く形へ整理した
  - `↺` は preset 継承中は disabled、上書き中だけ enabled とし、hover text で「preset 値へ戻す / 継承中」を説明することで行数を増やさず状態を伝えるようにした
  - style / gradient / effect / entrance の reset 導線は既存の継承フラグと save/reopen semantics を維持したまま icon button 化し、`preset_reset_ui_uses_compact_icon_without_status_rows` で旧文言の後退を防いだ
  - 今回の確認として `cargo test -q -p pauseink-renderer reveal_head_effect_tail_length_remains_usable_in_downscaled_preview -- --nocapture`、`cargo test -q -p pauseink-renderer reveal_head_effect_stays_visible_when_preview_is_downscaled -- --nocapture`、`cargo test -q -p pauseink-renderer -- --nocapture`、`cargo test -q -p pauseink-app resetting_head_effect_to_preset_restores_or_disables_it -- --nocapture`、`cargo test -q -p pauseink-app resetting_gradient_to_preset_restores_or_disables_it -- --nocapture`、`cargo test -q -p pauseink-app preset_reset_ui_uses_compact_icon_without_status_rows -- --nocapture`、`cargo fmt --all`、`cargo test -q -p pauseink-app --lib --bins`、`cargo test --workspace`、`cargo check -p pauseink-app --all-targets` を再通過した
  ...
  - auto-group chain は style / entrance / page change に加え、guide 基準更新、guide 解除、template reset / 配置確定、clear、undo / redo で切るようにした
  - `cargo test -p pauseink-app --lib auto_group_ -- --nocapture`、`cargo test -p pauseink-app --lib grouping_selected_groups_merges_members_without_nesting -- --nocapture`、`cargo test -p pauseink-app --bin pauseink-app capture_guide_from_object_breaks_auto_group_chain -- --nocapture`、`cargo test -p pauseink-app --bin pauseink-app reset_template_slots_breaks_auto_group_chain -- --nocapture` を通した
  ...
  - `V1-14` に着手し、template の横幅は `egui` shaping のまま、縦位置だけ font metrics と fallback の下揃えで差し替える方針を再確認した
