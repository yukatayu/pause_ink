...
### V1-12: template slot UI removal

**優先度:** P1
**依存:** V1-11
**ひとことで言うと:** template 側に露出している slot の UI 概念を引っ込め、`前スロット/次スロット` ボタンと current slot の可視化を削除する task。

**目的:** template は「並んだ下敷きを順に手書きでなぞる」用途が中心であり、slot の選択状態や index を UI に出さなくても十分使える。そこで、slot は内部実装 detail に戻し、利用者に見える UI からは `前スロット/次スロット`、`slot: 3/8` 相当の状態表示、current slot の強調表示を外す。

**具体的に困る場面**

- 左パネルが狭いと `テンプレート配置 / 前スロット / 次スロット / テンプレート解除` の 4 連ボタンが最小幅を押し上げる。
- 利用者から見ると `前スロット/次スロット` の用途が見えにくく、「何を動かしているのか」が分かりづらい。
- current slot の強調 box も、利用者にとっては単に「一つだけ色が違う」ノイズになりやすい。

**現状の問題**

- 現状の `slot index` は次 stroke の target を指すが、その意味が UI 文言に出ていない。
- `前スロット/次スロット` は index の加減算でしかなく、template の主用途に対して説明コストの高い UI になっている。
- current slot 強調表示は、内部の state machine をそのまま利用者へ漏らしている。
- 常時 primary row に置かれており、左パネルの横幅を無駄に押し広げている。

**設計**

- `テンプレート配置` と `テンプレート解除` だけを template action row に残す。
- `前スロット/次スロット` ボタンは削除する。
- `slot: 3/8` のような状態表示は入れない。
- current slot の box 強調表示も削除する。
- 内部実装として `current_slot_index` が残ること自体は許容するが、利用者向け UI 概念としては露出しない。
- template の描画順や自動進行ロジックは、この task では原則変えない。目的は UI の簡素化であり、capture semantics の変更ではない。
- guide/grid 系には一切波及させない。`GuideOverlayState`、guide gap、guide advance、guide highlight は out of scope と明記する。

**着手前に決めるべきこと**

- current slot 強調表示を完全削除するか、debug build のみ残すか。後から戻すと preview test をやり直す。
- 内部の `current_slot_index` をそのまま維持するか、将来の整理に備えて helper の責務だけ薄くするか。実装コストが変わる。
- template 多画文字の実運用で本当に困らないかを、実機確認でどこまで見るか。manual の説明量に影響する。
...
1. 左ペインから `前スロット/次スロット` ボタンを削除する。
2. preview 上の current slot 強調表示を削除する。
3. `テンプレート配置 / テンプレート解除` のみで主 row を構成し、panel 最小幅を確認する。
4. guide/grid 系に回帰が出ていないことを確認する。

**必要テスト**

- `app`: `前スロット/次スロット` ボタン削除後も template 配置 UI が崩れない
- `app`: template preview が current slot 非強調でも描画できる
- `app`: guide/grid の既存回帰 test がそのまま通る
- `app`: narrow panel でも template action row が崩れにくい

**完了条件**

- template UI から `前スロット/次スロット`、current slot 状態表示、current slot 強調表示を外せる
- 左パネルの template action row が軽くなる
- guide/grid の現在仕様へ影響を出さない
